...
public class Main extends Screen {
	...
	private RectF licensesRect[] = new RectF[2];
	...
	public Main() {
		...
		if (Constants.license) {
			...
			LargeIconsPosition[3] = new Point(w / 4 * 3,
					(int) RectFs[3].centerY());
			...
			super.iconca = null;
			licensesRect[0] = new RectF(0, h / 2 - 70, w, h / 2 - 10);
			licensesRect[1] = new RectF(0, h / 2 + 10, w, h / 2 + 70);
		}
	...
	public boolean onTouch(MotionEvent event) {
		...
		if (Constants.license) {
			...
			if (licensesRect[1].contains(event.getX(), event.getY())) {
				try {
					Constants.context.startActivity(new Intent(
							Intent.ACTION_VIEW, Uri
									.parse("market://details?id="
											+ "com.andraft.blacklistpro")));
		...
		if (Constants.license) {
			...
			for (int j = 0; j < LargeIconsPosition.length; j++) {
				...
				if (j == 4) {
					...
					canvas.drawText(Res.getString(iconText[j]),
							w / 2 - Res.getInteger(R.integer.largeIconWidth)
									* 1.7f, LargeIconsPosition[j].y
									+ WhiteTextSmall.getTextSize() / 2,
							WhiteTextSmall);
				} else if (j == 5) {
					...
					canvas.drawText(Res.getString(iconText[j]),
							LargeIconsPosition[j].x, LargeIconsPosition[j].y
			...
			canvas.drawText(" " + blockedCountSms, mt,
					dy + Res.getInteger(R.integer.smallIconWidth) / 3,
					WhiteText);
			...
			canvas.drawRect(licensesRect[0], WhiteRamca);
			canvas.drawText(Res.getString(R.string.license),
					licensesRect[0].centerX(), licensesRect[0].centerY()
							+ (WhiteText.descent() - WhiteText.ascent()) / 4,
					WhiteText);
			canvas.drawRect(licensesRect[1], WhiteRamca);
			canvas.drawText(Res.getString(R.string.download),
					licensesRect[1].centerX(), licensesRect[1].centerY()
							+ (WhiteText.descent() - WhiteText.ascent()) / 4,
					WhiteText);
		}
	}
